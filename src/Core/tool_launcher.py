...
def initialize(newModuleImportPath: str, args: DictToObject, nodeOutputPath:Path, toolsPath:Path):
...
def processData(moduleImportPath: str, args: dict, nodeOutputPath:Path, toolsPath:Path):
    argsObj = DictToObject(args)
    tool, argsObj = initialize(moduleImportPath, argsObj, nodeOutputPath, toolsPath)
    result = None
    if hasattr(tool, 'processData') and callable(tool.processData):
        with safe_chdir(nodeOutputPath):
            result = tool.processData(argsObj)
    return result
...
def processAllData(moduleImportPath: str, argsList: list[dict], nodeOutputPath:Path, toolsPath:Path):
    ...
    argsObjList = [DictToObject(args) for args in argsList]
    tool, _ = initialize(moduleImportPath, argsObjList[0], nodeOutputPath, toolsPath)
    nodeOutputPath.mkdir(exist_ok=True, parents=True)
    if hasattr(tool, 'processAllData') and callable(tool.processAllData):
        with safe_chdir(nodeOutputPath):
            result = tool.processAllData(argsObjList)
    return result