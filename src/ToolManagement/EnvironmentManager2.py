...
class EnvironmentManager:
    def __init__(self, envs_dir: str = "micromamba/envs"):
        self.base_dir = Path("./micromamba")
    ...
    def _get_log_path(self, env_name: str) -> Path:
        log_dir = self.envs_dir / env_name
    ...
    def _run(self, args: List[str], env_name: str, capture_output: bool = False):
        full_cmd = [self.micromamba_path] + args + ["--root-prefix", str(self.envs_dir)]
    ...
    def exists(self, name: str) -> bool:
        result = subprocess.run(
            [self.micromamba_path, "env", "list", "--json", "--root-prefix", str(self.envs_dir)],
    ...
    def launch(self, name: str, custom_command: str):
        ...
        command = [
            self.micromamba_path, "run", "-n", name, "bash", "-c", custom_command,
            "--root-prefix", str(self.envs_dir)
        ]
